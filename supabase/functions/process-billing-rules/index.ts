  ...
  try {
    ...
    for (const [tenant_id, tenantRules] of Object.entries(rulesByTenant)) {
      ...
      for (const rec of receivables) {
        ...
        const matchingRules = (tenantRules as any[]).filter(
          (r) => r.days_offset === diffDays,
        )
        ...
        for (const rule of matchingRules) {
          ...
          let message =
            rule.template ||
            'Olá {{nome}}, você tem um título no valor de {{valor}} vencendo/vencido em {{vencimento}}'
          ...
          localDueDate.setMinutes(
            localDueDate.getMinutes() + localDueDate.getTimezoneOffset(),
          )
          message = message.replace(
            /\{\{vencimento\}\}/g,
            localDueDate.toLocaleDateString('pt-BR'),
          )
          ...
          if (rule.channel === 'email' && customer.email) {
            ...
            console.log(
              `Simulando envio de ${rule.channel} para ${customer.name}`,
            )
            sent = true
          } else if (rule.channel === 'email' && !customer.email) {
            console.log(
              `Cliente ${customer.name} não possui email cadastrado para receber a notificação.`,
            )
          }
